...
namespace SouNews.Common {
    ...
    public class ImageHelper {
        ...
        /// <param name="color">颜色</param>
        /// <param name="location">水印位置</param>
        public static byte[] Watermark(string path, string[] letter, string[] location) {
            string kz_name = Path.GetExtension(path);
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg") {
                Image img = Bitmap.FromFile(path);
                Graphics gs = Graphics.FromImage(img);
                try {
                    Font font = new Font("微软雅黑",float.Parse("3.3"),FontStyle.Bold);
                    Brush br = new SolidBrush(Color.Black);
                    for (int i = 0; i < letter.Length; i++) {
                        string locaX = location[i].ToEString().Split(':')[0];
                        string locaY = location[i].ToEString().Split(':')[1];
                        gs.DrawString(letter[i], font, br, float.Parse(locaX), float.Parse(locaY));
                    }
